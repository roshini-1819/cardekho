...
const ROWS: Row[] = [
  ...
  { label: "Body", get: (c) => bodyLabel[c.bodyType], best: null },
  { label: "Fuel", get: (c) => fuelLabel[c.fuelType], best: null },
  { label: "Gearbox", get: (c) => transmissionLabel[c.transmission], best: null },
          ...
          {ROWS.map((row) => {
            ...
            return (
              <tr key={row.label} className="border-t border-white/5">
                <td className="sticky left-0 bg-[#0e1830] p-3 text-slate-300">{row.label}</td>
                {cars.map((c, i) => (
                  <td
                    key={c.id}
                    className={`p-3 ${
                      winner === i ? "font-semibold text-emerald-300" : "text-slate-100"
                    }`}
                  >
                    {row.get(c)}
                    {winner === i && " ✓"}
